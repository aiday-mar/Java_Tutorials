...
class ParallelMergeSorter {
    ...
    /* returns sorted array */
    public int[] sort() {
        int numWorkers = Runtime.getRuntime().availableProcessors(); // the number of workers is the number of processors
        ForkJoinPool pool = new ForkJoinPool(numWorkers);  // here we have created a fork join pool
    ...
    private class ParallelWorker extends RecursiveAction {
        ...
        protected void compute() {
            if (left < right) {
                int mid = (left + right) / 2; // find the middle point, we subdivide the array further
                ...
                merge(left, mid, right); // merge the two sorted sub arrays into one 
            }
        }
    }
...
The last code that we want to write is one for downloading images in java.
